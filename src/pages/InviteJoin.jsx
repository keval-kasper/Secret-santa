...
function InviteJoin() {
  ...
  useEffect(() => {
    const join = async () => {
      ...
      const emailExisting = await getDocs(
        query(
          collections.participants(),
          where("eventId", "==", eventId),
      ...
      const alreadyJoined = !existing.empty || !emailExisting.empty;
      if (!alreadyJoined) {
        ...
        setStatus({
          type: "success",
          message: "Joined! You can now draw a name.",
        });
      } else {
  ...
  if (!user) {
    return (
      <div className="card">
        <h2>Join event</h2>
        <p className="muted">
          Log in with the email the organizer invited, then revisit this link.
        </p>
      </div>
    );
  }
